...
export function Tracker() {
  ...
  const handleNavbarTabChange = (navbarTabId: TabId) =>
    setCurrentTab(navbarTabId.toString());

  const DemoDataCallout = () => {
    if (!isAuthenticated) {
      return (
        <Callout title="Demo Data" intent={Intent.WARNING}>
          This data is for demonstration purposes only and will not be saved. To
          track your own data, please sign up for an account.
        </Callout>
      );
    } else {
      return <div />;
