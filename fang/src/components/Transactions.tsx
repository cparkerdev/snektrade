...
export function Transactions() {
  ...
  const getTransSymbolText = (t: TableTransactionUI) => {
    if (t.TransType === 0) {
      ...
      return `${t.Symbol}-${partYear}-${
        tranDate.getMonth() + 1
      }-${tranDate.getDate()}-${getTransTypeText(t).substr(0, 1)}${t.Strike}`;
    }
  };

  const createdCellRenderer = (rowIndex: number) => {
    return (
      <Cell>{new Date(trans[rowIndex].CreatedAt).toLocaleDateString()}</Cell>
    );
  };
  ...
  const qtyCellRenderer = (rowIndex: number) => {
    return (
      <EditableCell
        ...
        value={trans[rowIndex].Quantity.toString()}
      />
    );
  };
  ...
  const priceCellRenderer = (rowIndex: number) => {
    return (
      <EditableCell
        style={{ textAlign: 'right' }}
        value={trans[rowIndex].Price.toFixed(2)}
      />
    );
  };
  ...
  const commCellRenderer = (rowIndex: number) => {
    return (
      <EditableCell
        style={{ textAlign: 'right' }}
        value={trans[rowIndex].Commission.toFixed(2)}
      />
    );
  };
  ...
  const feeCellRenderer = (rowIndex: number) => {
    return (
      <EditableCell
        style={{ textAlign: 'right' }}
        value={trans[rowIndex].Fees.toFixed(2)}
      />
    );
  };
  ...
  const amountCellRenderer = (rowIndex: number) => {
    return (
      <Cell style={{ textAlign: 'right' }}>
        {calcTransAmount(trans[rowIndex]).toFixed(2)}
      </Cell>
    );
  };
