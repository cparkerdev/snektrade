...
export type SimpleOptionsProps = { trade: TradeUI, setTrade: React.Dispatch<React.SetStateAction<TradeUI>> };
export function SimpleOptionsForm(props: SimpleOptionsProps) {
    ...
    return(
    <div>
        <Switch labelElement={"Short"} defaultChecked={transData.IsShort} onChange={onShortChange} inline={true} alignIndicator={Alignment.RIGHT} />
        <FormGroup
        label="Quantity"
        labelFor="quantity-input"
        labelInfo=""
        >
        <InputGroup id="quantity-input" placeholder="Enter Quantity" defaultValue={transData.Quantity.toString()} onChange={onQuantityChange} />
        </FormGroup>
        <FormGroup
        label="Price"
        ...
        labelInfo="($)"
        >
        <InputGroup id="price-input" placeholder={"0.00"} onChange={onPriceChange} />
        </FormGroup>
        <FormGroup
        label="Strike"
        labelFor="strike-input"
        labelInfo="($)"
        >
        <InputGroup 
            id="strike-input" 
