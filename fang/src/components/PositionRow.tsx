...
*/
  ...
  const getLotSymbolText = (l: LotUI) => {
    if (l.Strategy === 0) {
      ...
      return `${l.Symbol}-${partYear}-${
        tranDate.getMonth() + 1
      }-${tranDate.getDate()}-${getLotTypeText(l).substr(0, 1)}${l.Strike}`;
    }
  };
  ...
  const rowMenu = (
    <Menu>
      <MenuItem
        text={`Buy${props.l.IsShort ? ' Back' : ' More'}`}
        onClick={() => openBuyTrade()}
      />
      <MenuItem
        text={`Sell${props.l.IsShort ? ' More' : ' Back'}`}
    ...
    <React.Fragment>
      <tr key={props.index}>
        <td>
          ...
          <span style={{ margin: '0px 5px 0px 5px', verticalAlign: 'middle' }}>
            {getLotSymbolText(props.l)}
          </span>
        </td>
        <td
          style={{
        ...
        >{`${props.l.IsShort ? '-' : ''}${props.l.Quantity}`}</td>
        <td
          style={{
            margin: '0px 0px 0px 0px',
            verticalAlign: 'middle',
